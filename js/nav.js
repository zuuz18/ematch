...
(function () {
  ...
  function loadNavCache() {
    try {
      ...
      document.querySelectorAll('.sos-balance-display').forEach(el => { el.textContent = SOS; });
...
// Holds the active Firestore unsubscribe fn — prevents stacking listeners
...
function renderMatchCard(id, m) {
  ...
  return `
    <div class="match-card" data-id="${id}" role="listitem" tabindex="0"
      aria-label="${m.platform} match, stake: ${m.stakeAmount} coins">
      ...
      <div class="match-card-body">
        ...
        <div class="match-footer">
          <div class="stake-amount">${window.sosCoin?window.sosCoin(m.stakeAmount||0,18):''} ${window.sosFormat(m.stakeAmount||0)} SOS</div>
    ...
    content.innerHTML = `
      ...
      <div class="grid-2 mb-md">
        <div class="card" style="text-align:center;padding:var(--sp-md)">
          <div style="font-size:22px;font-weight:900;color:var(--accent-green)">
            ${window.sosCoin?window.sosCoin(m.stakeAmount||0,18):''} ${window.sosFormat(m.stakeAmount||0)} SOS
          </div>
        ...
        <div class="card" style="text-align:center;padding:var(--sp-md)">
          <div style="font-size:22px;font-weight:900;color:var(--accent-gold)">
            ${window.sosCoin?window.sosCoin((m.stakeAmount||0)*2,18):''} ${window.sosFormat((m.stakeAmount||0)*2)} SOS
          </div>
      ...
      ${canJoin ? `
        ...
        <button class="btn btn-primary mt-sm" id="btn_join_match"
          ...
          <span class="btn-text">🎮 Ku Biir — ${window.sosCoin?window.sosCoin(m.stakeAmount||0,18):''} ${window.sosFormat(m.stakeAmount||0)} SOS</span>
...
async function handleCreateMatch(e) {
  ...
  if (stakeAmount < 8000)
    { showError('cm_stake', 'Ugu yaraan 8 kun SOS (≈ $0.25)'); valid = false; }
  if ((currentUserData?.sosBalance || 0) < stakeAmount)
...
async function setMatchWinner(matchId, winnerUid, m) {
  ...
  await runTransaction(db, async tx => {
    ...
    tx.update(lRef, {
      escrowSOS: Math.max(0, (lSnap.data().escrowSOS || 0) - m.stakeAmount)
    });
    const t1 = doc(collection(db, 'transactions'));
    tx.set(t1, { userId: winnerUid, type: 'match_win',  sos: +prize,       relatedMatch: matchId, createdAt: serverTimestamp(), meta: { matchId, loserUid } });
    const t2 = doc(collection(db, 'transactions'));
    tx.set(t2, { userId: loserUid,  type: 'match_loss', sos: -m.stakeAmount, relatedMatch: matchId, createdAt: serverTimestamp(), meta: { matchId, winnerUid } });
  });
}
      ...
      body: JSON.stringify({
        ...
        messages: [{
          ...
          content: [
            ...
            {
              type: 'text',
              text: `This is a game result screenshot from a 1v1 match.
Player 1 in-game username: "${myUsername}"
Player 2 in-game username: "${oppUsername}"

Look at the final score/result screen and determine who won.
...
- confidence: "high" if result is clearly visible, "low" if blurry or unclear`
            }
          ]
        }]
      })
...
async function handleDeposit(e) {
  ...
  try {
    ...
    if (ussdAmount) ussdAmount.textContent = `$${amount} →  ${window.sosFormat(sosAmount)} SOS shilling`;
...
async function handleSendCoins(e) {
  ...
  if (sendAmount > (currentUserData?.sosBalance || 0))
  ...
  try {
    await runTransaction(db, async tx => {
      ...
      tx.set(t1, {
        ...
        sos: -sendAmount, relatedMatch: null,
    ...
    showToast(`✅  ${window.sosFormat(sendAmount)} la diray!`, 'success');
      ...
      return `
        <div class="tx-item" role="listitem">
          ...
          <div class="tx-amount ${isCredit ? 'credit' : 'debit'}">
            ${isCredit ? '+' : ''}${(window.sosFormat(t.sos||0))} ${'SOS'}
          </div>
        </div>`;
    }).join('');
  });
      ...
      return `
        <div class="deposit-req-card">
          <div class="deposit-req-meta">
            <span class="deposit-req-coins"> ${window.sosFormat(r.sosAmount||0)}</span>
          ...
          <div class="grid-2">
            <button class="btn btn-primary btn-sm"
              onclick="adminApproveDeposit('${d.id}','${r.userId}',${r.sosAmount})">
              ✅ Ogolow
            </button>
...
window.adminApproveDeposit = async function(reqId, userId, sosAmount) {
  if (!confirm(
    `✅ Deposit ogolaan?\n ${window.sosFormat(sosAmount)} SOS\n$${(sosAmount*0.1).toFixed(2)}`
  )) return;
  if (!requireOnline()) return;
  ...
  try {
    await runTransaction(db, async tx => {
      ...
      tx.update(userRef, {
        sosBalance: (uSnap.data().sosBalance || 0) + sosAmount
      });

      const txRef = doc(collection(db, 'transactions'));
      tx.set(txRef, {
        userId, type:'deposit_approved', sos: sosAmount,
      ...
      tx.set(logRef, {
        ...
        targetUserId: userId, reqId, sosAmount,
        createdAt: serverTimestamp()
      });
    });
  ...
  try {
    await runTransaction(db, async tx => {
      ...
      tx.set(t1, {
        ...
        sos: +prize, relatedMatch: matchId,
      ...
      tx.set(t2, {
        ...
        sos: -m.stakeAmount, relatedMatch: matchId,
...
async function loadProfileStats(uid) {
  try {
    ...
    // Deduplicate in case of edge cases (user can't be both creator & joiner)
    const createdIds = new Set(createdSnap.docs.map(d => d.id));
    const joinedIds  = joinedSnap.docs.map(d => d.id).filter(id => !createdIds.has(id));
    const totalMatches = createdSnap.size + joinedIds.length;
    ...
    if (statCoins)   statCoins.textContent   = (window.sosFormat(currentUserData?.sosBalance||0));
    if (statMatches) statMatches.textContent = totalMatches;
    if (statWins)    statWins.textContent    = totalWins;
    if (statEscrow)  statEscrow.textContent  = (window.sosFormat(currentUserData?.escrowSOS||0));
  } catch (err) {
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (page === 'admin.html') {
    ...
    // Real-time listener for admin page user data
  ...
  if (page === 'wallet.html') {
    ...
    function updateWalletDisplay() {
      ...
      if (coinsEl)  coinsEl.textContent  = window.sosFormat(bal);
      if (escrowEl) escrowEl.textContent = window.sosFormat(esc);
    ...
    document.getElementById('dep_amount')?.addEventListener('input', function() {
      ...
      if (preview) preview.textContent = coins > 0 ? window.sosFormat(coins) : '—';
      if (hint)    hint.textContent    = coins > 0
        ? ` ${window.sosFormat(coins)} kun SOS la helayaa` : 'SOS la helayo: —';
    });
